...
module auto_renewal::vault_tests {
    ...
    fun test_withdraw_as_beneficiary() {
        ...
        next_tx(&mut s, U);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
        vault::withdraw(&mut v, 30_000_000, ctx(&mut s));
        test_scenario::return_shared(v);
        next_tx(&mut s, U);
        let v = test_scenario::take_shared<RenewalVault>(&mut s);
    ...
    fun test_withdraw_as_non_beneficiary_fails() {
        ...
        next_tx(&mut s, K);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
        vault::withdraw(&mut v, 1000, ctx(&mut s));
        test_scenario::return_shared(v);
        end(s);
    }
    ...
    fun test_execute_renewal() {
        ...
        vault::create_vault(&config, blob(&mut sys, 50, ctx(&mut s)), wal(100_000_000, ctx(&mut s)), 5, 10, option::none(), ctx(&mut s));
        unit_test::destroy(sys);
        put_config(&mut s, config);
        next_tx(&mut s, K);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
    ...
    fun test_execute_renewal_not_due_fails() {
        ...
        let config = get_config(&mut s);
        let mut sys = sys(ctx(&mut s));
        vault::create_vault(&config, blob(&mut sys, 200, ctx(&mut s)), wal(100_000_000, ctx(&mut s)), 5, 10, option::none(), ctx(&mut s));
        unit_test::destroy(sys);
        put_config(&mut s, config);
        next_tx(&mut s, K);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
    ...
    fun test_execute_renewal_insufficient_balance() {
        ...
        let config = get_config(&mut s);
        let mut sys = sys(ctx(&mut s));
        vault::create_vault(&config, blob(&mut sys, 50, ctx(&mut s)), wal(1_000, ctx(&mut s)), 5, 10, option::none(), ctx(&mut s));
        unit_test::destroy(sys);
        put_config(&mut s, config);
        next_tx(&mut s, K);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
    ...
    fun test_execute_renewal_cap_reached() {
        ...
        let config = get_config(&mut s);
        let mut sys = sys(ctx(&mut s));
        vault::create_vault(&config, blob(&mut sys, 50, ctx(&mut s)), wal(100_000_000, ctx(&mut s)), 5, 10, option::some(55), ctx(&mut s));
        unit_test::destroy(sys);
        put_config(&mut s, config);
        next_tx(&mut s, K);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
    ...
    fun test_destroy_vault() {
        ...
        vault::withdraw(&mut v, bal, ctx(&mut s));
        test_scenario::return_shared(v);
        next_tx(&mut s, U);
        let mut v = test_scenario::take_shared<RenewalVault>(&mut s);
        vault::reclaim_blob(&mut v, ctx(&mut s));
    ...
    fun test_create_vault_fails_when_paused() {
        ...
        let config = test_scenario::take_shared<FeeConfig>(&mut s);
        let mut sys = sys(ctx(&mut s));
        vault::create_vault(&config, blob(&mut sys, 100, ctx(&mut s)), wal(100_000_000, ctx(&mut s)), 5, 10, option::none(), ctx(&mut s));
        test_scenario::return_shared(config);
        unit_test::destroy(sys);
        end(s);
    }
