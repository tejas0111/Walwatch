...
import { SuiJsonRpcClient, SuiObjectResponse, PaginatedObjectsResponse } from '@mysten/sui/jsonRpc';
...
export class VaultScanner {
  ...
  /**
   * @param pool                  Sui RPC client pool with multi-endpoint failover
   ...
   */
  private callClient<T>(fn: (client: SuiJsonRpcClient) => Promise<T>): Promise<T> {
  ...
  async findDueVaults(): Promise<DueVault[]> {
    ...
    try {
      ...
      logger.warn({
        service: 'sui-rpc',
        operation: 'findDueVaultsPrimary',
        error: (error as Error).message,
        fallbackEnabled: this.enableEventFallback,
      ...
      try {
        ...
        logger.error({
          service: 'sui-rpc',
          operation: 'findDueVaultsFromEvents',
   ...
   *
   * Uses the raw `suix_queryObjects` RPC method via SuiJsonRpcClient.call()
  ...
  private async queryAllVaults(): Promise<DueVault[]> {
    ...
    while (hasMore && vaults.length < this.maxVaults) {
      ...
      const response: SuiPage = await this.callClient((c) =>
        c.call<SuiPage>(
          'suix_queryObjects',
          [
            {
  ...
  private async findDueVaultsFromEvents(): Promise<DueVault[]> {
    ...
    try {
      ...
      const events = await this.callClient((c) =>
        c.queryEvents({
  ...
  private async fetchVaultsByIds(ids: string[]): Promise<DueVault[]> {
    ...
    try {
      const objects = await this.callClient((c) =>
        c.multiGetObjects({
          ids,
      ...
      logger.error({
        ...
        error: (error as Error).message,
        timestamp: new Date().toISOString(),
      }, 'Failed to fetch vaults by IDs');
      return [];
    }
  }

  /**
   * Fetch a single vault by its on-chain object ID and parse into DueVault.
   * Returns null if the object is not found or cannot be parsed.
   */
  async findVaultByObjectId(objectId: string): Promise<DueVault | null> {
    const vaults = await this.fetchVaultsByIds([objectId]);
    return vaults[0] || null;
  }

  /**
   * Find all vaults with pending withdrawals whose delay has elapsed.
   */
  async findPendingWithdrawals(): Promise<Array<{
    objectId: string;
    ...
    if (!this.packageId) {
      logger.warn('PACKAGE_ID not set — returning empty pending withdrawals');
      return [];
    }

    try {
      const allVaults = await this.queryAllVaults();

      const vaultIds = allVaults.map(v => v.objectId);
      ...
      for (const obj of objects) {
        ...
        const amount = Number(fields.pending_withdraw_amount || 0);
        if (amount > 0) {
          pending.push({
            objectId: obj.data.objectId,
            pendingWithdrawAmount: amount,
            pendingWithdrawInitEpoch: Number(fields.pending_withdraw_init_epoch || 0),
            withdrawDelayEpochs: Number(fields.withdraw_delay_epochs || 0),
          });
        }
      }
      ...
      logger.error({ error, packageId: this.packageId }, 'Failed to find pending withdrawals');
      return [];
    }
  }
  ...
  private async getCurrentEpoch(): Promise<number> {
    const seq = await this.callClient((c) => c.getLatestCheckpointSequenceNumber());
    const checkpoint = await this.callClient((c) => c.getCheckpoint({ id: seq }));
