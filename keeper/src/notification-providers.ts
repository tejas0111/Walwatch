    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        logger.warn({
          service: 'webhook',
          endpoint: new URL(url).hostname,
          statusCode: (error as { status?: number; message?: string }).status,
        ...
        return {
          ...
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto;">
              ...
              <p>Your auto-renewal vault <strong>${escapeHtml(event.vaultId.slice(0, 10))}...</strong> does not have enough WAL to renew blob storage.</p>
              <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
                <tr><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">Blob ID</td><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;"><code>${escapeHtml(event.blobId.slice(0, 16))}...</code></td></tr>
        ...
        return {
          ...
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto;">
              ...
              <p>Your auto-renewal policy for blob <strong>${escapeHtml(event.blobId.slice(0, 16))}...</strong> has reached its maximum total epochs cap.</p>
              <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
