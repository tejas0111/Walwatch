...
export class SuiClientPool {
  ...
  constructor(config: SuiClientPoolConfig) {
    ...
    this.entries = config.urls.map((url) => ({
      client: new SuiJsonRpcClient({ url, network: 'testnet' }),
      url,
  ...
  get primaryClient(): SuiJsonRpcClient {
   ...
   *
   * @param fn  Async function receiving a SuiJsonRpcClient
   * @returns   The result of fn
   */
  async call<T>(fn: (client: SuiJsonRpcClient) => Promise<T>): Promise<T> {
