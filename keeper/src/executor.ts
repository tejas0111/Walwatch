...
export class RenewalExecutor {
   ...
   * Returns null if the delay has not yet elapsed.
   */
  async finalizeWithdraw(vault: {
    objectId: string;
    pendingWithdrawAmount: number;
    pendingWithdrawInitEpoch: number;
    withdrawDelayEpochs: number;
  }): Promise<string | null> {
    const currentEpoch = await this.callClient((c) => c.getCurrentEpoch().then(r => Number(r.epoch)));
    const availableEpoch = vault.pendingWithdrawInitEpoch + vault.withdrawDelayEpochs;
    if (currentEpoch < availableEpoch) return null;

    const tx = new Transaction();
    tx.setSender(this.keypair.getPublicKey().toSuiAddress());
    tx.moveCall({
      target: `${this.packageId}::vault::finalize_withdraw`,
