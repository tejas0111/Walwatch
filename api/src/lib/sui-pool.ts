 ...
 * The API uses this instead of a raw SuiClient for RPC calls that
...
export class SuiClientPool {
  ...
  constructor(urls: string[], options: SuiPoolOptions = {}) {
    ...
    for (const url of urls) {
      ...
      if (trimmed) {
        ...
        this.entries.push({
          client: new SuiClient({ url: validated }),
          url: validated,
          state: 'CLOSED',
          failureCount: 0,
          lastFailureTime: 0,
    ...
    if (this.entries.length === 0) {
      ...
      this.entries.push({
        client: new SuiClient({ url: defaultUrl }),
        url: defaultUrl,
        state: 'CLOSED',
        failureCount: 0,
        lastFailureTime: 0,
   ...
   */
  async call<T>(fn: (client: SuiClient) => Promise<T>, signal?: AbortSignal): Promise<T> {
